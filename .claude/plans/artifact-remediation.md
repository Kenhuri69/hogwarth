...
- [x] Lot C — `voix_rowena_relique` +`bonusMag:2` (budget 140→210, parité reliques vocales) + desc « INT+4 MAG+2 » ✅ 2026-06-15 — sim endgame kit-complet sain (Ch.13 OK, +2 MAG sous résolution sim), `scenarioVoiceRelics` vert ; cache-bump data v40→41 / CACHE_VERSION v155→156
- [x] Lot B — Premium Gryffondor (`orbe_runique_premium_gryff`) rehaussée à **490** de budget (B1, base `orbe_runique` intacte) ✅ 2026-06-15 — `bonusMag:6` (210) + `bonusElemDmg:{tous:0.20}` (160) + `bonusCritChance:5` (60) + `bonusLck:3` (60) ; parité bande 480-560 (Slyth 492 / Pouf 510). **Écart vs exemple du plan** : `bonusInt:3` → `bonusLck:3` (budget identique) pour éviter l'interférence D1 (INT→MAG) avec l'assertion equip-delta du test, et coller à l'identité crit de Gryffondor. Test `scenarioPremiumReward` : conformité précuit assouplie `===`→`>=` (une Premium est AU MOINS le précuit standard, jamais moins — sanctionne le hand-tuning B1). `HOUSE_PREMIUM`/Codex/icône **intacts** (pas de B2). Sim Gryffondor kit-complet (Apothéose+set+Forge5+25niv+artefacts) sain (Ch.13 OK, décroissance naturelle). cache-bump data v41→42 / CACHE_VERSION v156→157.
- [x] Lot D — repricing (hausses seules) ✅ 2026-06-15 — `chapeau_pointu` 200→**300** (rare acte II), `bottes_dragon` 340→**600** (rare acte III) ; `amulette` **laissée** à 250 (Q1 = laisser, rampe early). Aucune baisse. Pas de sim (prix ≠ puissance). `scenarioShopLimits` vert ; cache-bump data v42→43 / CACHE_VERSION v157→158.
- [x] Lot E — **découpé E1/E2** (les deux livrés).
  - [x] **E1** — 5 uncommons neufs ✅ 2026-06-15 : `serre_tete_etude` (head, MAG1/INT1/DEF1, Serdaigle), `plastron_renforce` (body, DEF2/END1, Poufsouffle), `bottes_lestes` (feet, AGI2/LCK1), `cape_doublee` (cloak, DEF1/AGI1/END1), `ceinture_etudiant` (belt, DEF1/LCK1/END1, Poufsouffle) — budgets 90/90/75/75/90, prix 180/200/170/180/170, `SHOP_CATALOG` minFloor 3-4. Icônes : painterly dédiées (icon_factory recettes tiara/boot/belt) pour serre_tete/bottes_lestes/ceinture_etudiant ; repli legacy réutilisé pour les 5 (+ plastron→robe1, cape→cape_voyageur, convention robe_combat/cape_combat car pas de part SVG body/cloak). **type** = convention dominante par slot (head/feet/cloak/belt `acc`, body `armor`) — léger écart vs note « armor défensifs » du plan, priorité back-compat slot. Sim mid-game sain (uncommons sous la puissance des rares → ne lèvent pas le plafond) ; `scenarioShopLimits`/`scenarioItemIcons` (176 items mappés)/`scenarioHouseFavorShop` verts ; cache-bump data v43→44 / shop v15→16 / item-icons v27→28 / CACHE_VERSION v159→160 (rebase : master avait atteint v159 entre-temps).
  - [x] **E2** — belt epic `ceinture_aurors` ✅ 2026-06-16 : belt epic `DEF+3 END+3 bonusCritChance+4` (budget 213), `price:900`, tint or, `SHOP_CATALOG` minFloor 9 (Hogsmeade corrompu) — 1er epic du slot `belt` (étude §5.2/§6.7). Icône painterly dédiée (recette `ceinture_aurors`, part belt, rareté epic) + repli legacy `ceinture_alchimiste.png`. Sim endgame kit-complet inchangé (epic sous le best-in-slot légendaire du sim → plafond Ch.13 intact). `scenarioShopLimits`/`scenarioItemIcons` (177 items) verts ; cache-bump data v44→45 / shop v16→17 / item-icons v28→29 / CACHE_VERSION v160→161.
