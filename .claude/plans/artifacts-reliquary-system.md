...
| **P3 (partie 2)** ✅ | **Premium exclusive + Codex méta-objectif** *(livré 2026-06-14)* | 4 Premium achetables **uniquement** au Marchand d'Ombre (`rarityScales` + `basePrice:9000` × `priceMultiplier 1.4` → 12 600 G, ×1.5ⁿ) ; Codex **« Reliques de la Mort »** (nouveau type de condition `allItems` : Sureau+cape_invis+anneau_resurrection ; complète le titre `maitreDeLaMort` déjà en place) | ✅ smoke `scenarioPremiumShadowVendor` + `DeathlyHallows` T5 (Codex allItems) verts ; cache bump codex v11 / data v38 / npcs v33 / `CACHE_VERSION v146` |
| **P3 (partie 3a)** ✅ | **PNJ Apothicaire des Reliques** *(livré 2026-06-14)* | Vendeur ambulant « Cassiopée Vance » (`random:true`, ét.6+), wares = 7 formes mid-game (orbes/cristal/gantelets/talisman/grimoire/masque), buyback artefacts 70 %, portrait Copilot Règle B (`img/npc/apothicaire_reliques.png`) | ✅ smoke `scenarioVendors` (existence/random/portrait/pool ét.6) ; cache npcs v34 / `CACHE_VERSION v147` |
| **P3 (partie 3b)** ✅ | **Reliques vocales + Chœur** *(livré 2026-06-14)* | 4 items `relique_vocale` §1.4 C (trinket, epic, non vendables, icônes Copilot épiques) octroyés **en voyant l'écho du Fondateur** (`grantVoiceRelicForEcho`, one-shot, hook défensif dans `floor-ambiance.js`) ; Codex **« Chœur des Fondateurs »** (`allItems` des 4). Déviations notées : « fearImmune partiel » (Godric) & « spellLifesteal » (Salazar) rendus en stats ; « régen /pas » (Helga) → `regenHp` /tour | ✅ smoke `scenarioVoiceRelics` (octroi one-shot + Chœur + icône) + `ItemIcons` (171) ; cache codex v12 / data v39 / floor-ambiance v11 / inventory-core v7 / item-icons v26 / `CACHE_VERSION v148` |
| **P3 (partie 3c)** ✅ | **Drops Premium** *(livré 2026-06-14)* | Obtention Premium hors Marchand d'Ombre via `grantHousePremiumDrop(source)` (one-shot tant que non possédée, mappe `chosenHouse`→Premium) : coffre des Ruines (≥14, 12 %, `openChest`) + boss epic Ténébreux (≥18, 20 %, `endBattle` bloc darkness) | ✅ smoke `scenarioHousePremiumDrop` (mapping 4 Maisons + one-shot) ; cache inventory-core v8 / movement-interactions v17 / battle-rewards v9 / `CACHE_VERSION v149` |
