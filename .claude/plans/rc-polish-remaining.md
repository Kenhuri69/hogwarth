...
| P3.3 | Refactor `monsters.js`/`data.js`/`npcs.js` en sous-fichiers | ✅ **Lot A fait (2026-06-21)** · Lots B/C différés | Plan : [`p33-data-split.md`](./p33-data-split.md). **Lot A livré** : `data.js` (1828 l.) découpé par couper-coller PUR en socle `data.js` (177 l.) + `data-characters.js`/`data-spells.js`/`data-items.js`/`data-world.js`. Zéro call-site touché, 946 units + 263 smoke + pwa-smoke verts, cache bumpé (v210). Lots B/C (`monsters.js`/`npcs.js`, mono-concern) **différés** comme recommandé. |
...
| P3.5 | Variété de boucle (playtest « 3 boucles ») | 📝 **protocole livré (2026-06-21)** | Hors-code par nature. Protocole humain complet (setup, fiche d'observation, données BalanceLog `loopDepths`, grille de décision → boutons `_BARK_COOLDOWN_MS`/variantes de beats) : [`docs/playtest-3-boucles.md`](../../docs/playtest-3-boucles.md). En attente d'exécution humaine ; aucun ajustement code tant que la lassitude n'est pas mesurée. |
