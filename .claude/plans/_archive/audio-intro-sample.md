...
| 2026-05-11 | Phase D — Zone 7-8 | Sample `the_stone_throat.mp3` (26,10 s stéréo 192) reçu → encodé en `audio/ambient_depths.ogg` (mono q2 = 187 KB). Branchement : 1 ligne dans `_ZONE_SAMPLES.depths`. Test HTTP floor=7 : buffer chargé (26,10 s), 1 source active, 0 warning |
| 2026-05-11 | Phase D — Zone 9+ (clôture) | Sample `a_breath_held_beneath.mp3` (26,93 s stéréo 192) reçu → encodé en `audio/ambient_abyss.ogg` (mono q2 = 196 KB). Branchement : 1 ligne dans `_ZONE_SAMPLES.abyss`. **Phase D terminée — 5 zones sur 5 livrées** (total 1004 KB). Test cascade HTTP floors=[1,3,5,7,10] : chaque zone charge son sample, 1 source maintenue tout le long, buffers cumulés en cache (intro+tension+dungeon+depths+abyss), 0 warning |
| 2026-05-11 | Phase E — Combat Normal | Sample `field_of_banners.mp3` (30,77 s stéréo 192) reçu → encodé en `audio/combat_normal.ogg` (mono q2 = 260 KB). Refonte du système de samples : `_loadZoneSample` factorisée en `_loadSample(key, url)` générique, `_playZoneSampleLoop` en `_playSampleLoop(key, isRelevant)` avec prédicat de stop. Nouveau registre `_COMBAT_SAMPLES = { combat_normal }` (Difficile / Expert non livrés → procédural). Helper `_combatSampleKey()` mappe la difficulté courante (`difficulty` global) à `combat_normal|combat_hard|combat_expert`. `startCombatMusic` devient un dispatcher sample/procédural ; `_playProceduralCombat` extrait du code existant sans changement de comportement. Filtre smoke élargi de `audio/ambient_*.ogg` à `audio/*.ogg`. Test HTTP : combat Normal joue le sample (30,77 s, 1 source), `stopCombatMusic` reprend l'ambient intro proprement, combat Expert tombe sur procédural sans warning |
