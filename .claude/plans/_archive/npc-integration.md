...
**Statut global** : itérations 1, 2, 4, 6, 7 (def + 7.4 logique métier) **livrées sur master**. Itération 3 portraits livrée sur branche `claude/npc-portraits-iter3` (cf. `.claude/plans/npc-portraits-iter3.md`) : (a) ✅ correction portrait Pomfresh (broche → caducée d'argent), (b) ✅ portraits PNG des 4 PNJ lore (Sir Nicolas, Moine Gras, Rusard, Trelawney). Plan officiellement clos.
...
| 2026-05-10 | Itération 6 (rebase post-#52/#53) | Rencontres PNJ aléatoires lore. 4 PNJ ajoutés (Sir Nicolas étage 1+, Moine Gras étage 2+, Rusard étage 1+, Trelawney étage 3+). Champ `dialogues.idleRandom: [...]` pour varier les répliques. Pool combiné vendeur+lore via `getRandomEncountersForFloor`, proba 35→50% par étage. `getRandomVendorsForFloor` restreint aux entrées avec `wares` (compat smoke). Portraits PNG TODO — emoji fallback (👻🍷🐈‍⬛🔮). Smoke 3septies (T1 registre, T2 pools cloisonnés, T3 dialog flow + idleRandom, T4 état quête). Rebase sur master post-merge #52/#53 : conflits résolus dans `index.html` (cache-bust npcs v=7, dungeon v=5, npc-dialog v=5), `npc-dialog.js _npcDialogPages` (chaîne de fallback idle : `idleSpent` > `contextualLore` > `idleRandom` > `idle`), `npcs.js` (commentaires schéma fusionnés), `tests/smoke.js` (renommage `Scénario 3quinquies` → `3septies` pour libérer le slot pris par `scenarioRepeatableQuestSpawn`). |
| 2026-05-12 | Itération 3 (portraits PNJ lore + correction Pomfresh) | Sur branche `claude/npc-portraits-iter3`. 5 PNG fournis par l'utilisateur via Nano Banana (1254×1254 chacun) : Pomfresh re-gen avec caducée d'argent au collet (correction de la broche en croix chrétienne de l'itération 2), Sir Nicolas (fantôme tête penchée), Moine Gras (fantôme tonsuré), Rusard (concierge + chat tigré sur l'épaule), Trelawney (lunettes loupes + tasse de thé). Pipeline simplifié vs monstres : pas de rembg car convention RGB sans alpha pour les portraits PNJ ; center-crop carré + resize 256×256 Lanczos via Pillow. Tailles finales 95–116 KB. 4 `portraitImg` ajoutés dans `js/npcs.js` (sir_nicolas, moine_gras, rusard, trelawney) ; pomfresh.png écrasé in-place. Smoke 34/34. Plan dédié `.claude/plans/npc-portraits-iter3.md`. Itération 3 et donc le plan global officiellement clos. |
