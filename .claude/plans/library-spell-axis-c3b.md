...
| 2026-05-30 | Re-vérification (branche `claude/library-spell-axis-c3b-RA3GV`, identique à `origin/master`). Code reconfirmé : `_spellForCaster` (power/focus/legacy combiné), `getSpellPath`, lock de voie au niv 0, migration idempotente `save.js` (`spellPaths = {}`). `scenarioLibraryUpgrade` T3 power (cost inchangé) / T4 focus (power inchangé) / T5 legacy combiné — tous verts. Suite complète **133/133** (exit 0). Aucun travail restant. |
