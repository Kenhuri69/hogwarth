...
function drawCorridor(cx, cy, scale, W, H) {
  ...
  for (let d = 1; d <= DEPTH; d++) {
    ...
    if (!pendingSprite) {
      ...
      // Jardin d'herbes (Potions P6.b3) : un jardin encore caché n'a pas de
      ...
      const _isCellSprite = (cell === CELL.CHEST || cell === CELL.STAIRS_D || cell === CELL.STAIRS_U || cell === CELL.SHOP || cell === CELL.NPC || cell === CELL.FORGE || cell === CELL.LIBRARY || cell === CELL.FOUNTAIN || cell === CELL.ALTAR || cell === CELL.RUNE || cell === CELL.STELE || cell === CELL.REQUIREMENT || _gardenVisible);
  ...
  if (pendingSprite) {
    ...
    else if (cell === CELL.REQUIREMENT) {
      const spent = (typeof usedRequirementRooms !== 'undefined') && usedRequirementRooms
        && usedRequirementRooms.has(`${pendingSprite.mapX},${pendingSprite.mapY}`);
      drawRequirementSprite(x, baseY, sz, spent);
    }
