...
| **Biais de génération par Maison — V2 (implémentation)** : pondération cosmétique de salles + skins de monstres thématiques selon `chosenHouse`, **power-neutral strict** (même grille Ch.13). Gate de release : **sim d'équilibrage neutre** (`tools/sim-difficulty.js` par Maison) confirmant 0 écart de win-rate ; flag de repli V1. Spec : 10 §10.6. **Livré partiel (2026-06-20, item 2c)** : levier **skin visuel** seul — livrée de Maison sur les cartes d'ennemi (`houseSkinClass`/`HOUSE_SKIN_ENABLED`, `battle-ui.js` + CSS), **power-neutral par construction** (rendu seul → 0 sim requis), testé `units.js`. **Pondération de salles toujours différée** derrière le gate sim (Item 3). | 🟡 Basse | 10, `battle-ui.js`, `style.css` (skin) ; `dungeon.js`, `dungeon-spawning.js` (salles, gated) | Élevée | Skin ✅ (2026-06-20) ; salles : sim figé |
