...
| Tâche | Priorité | Chapitres / systèmes | Complexité | Dépendances |
|-------|----------|----------------------|------------|-------------|
| **Boss-gardiens des Chambres des Fondateurs** (ét. 17-20) + illumination selon `chosenHouse` — **DÉCOUPÉ en 3 lots** : **Lot 1/3 ✅ Fait (2026-06-15)** = 4 boss-gardiens epic (`gardien_{lion,serpent,aigle,blaireau}`, thématisés par élément, drop = légende de Maison, `minFloor:17`, testés `units.js` §18) ; ⏳ Lot 2 = placement en chambre (spawn lié à l'illumination §10.5, `dungeon-spawning.js`) ; ⏳ Lot 3 = art PNG + promo/Codex. NB : l'illumination §10.5 + `FOUNDER_CHAMBERS` (étage-scène) **étaient déjà livrés** (`floor-ambiance.js`). | 🟠 Moyenne | 10, 11, `monsters.js`, `dungeon.js` | Élevée | Phase 2 |
