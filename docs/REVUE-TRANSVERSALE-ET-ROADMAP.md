...
| ✅ **Décisions `❓` endgame** (« ce qui dort », barks Ténébreux, biais Maison V2) | Tranché (2026-06-19, AskUserQuestion) : (1) personnifié = **le Dormeur** (10 §10.3, 11 §11.7.3, 03 §3.6) ; (2) barks `loopEcho` livrés (09 §VII.5) ; (3) biais Maison **V2 ouvert** power-neutral, **impl. close (2026-06-20)** : 3 leviers livrés (skin, perception, pondération de salles), gate d'équité satisfait (Phase 3, `DIFFICULTY_REPORT.md §8`) | ✅ Résolu |
...
| ~~**Décisions `❓` endgame** : « ce qui dort » personnifié ? barks Ténébreux ? biais génération par Maison (V2) ?~~ ✅ **Tranché (2026-06-19, AskUserQuestion)** : **(1)** « ce qui dort » **personnifié = *le Dormeur*** (présence primordiale, jamais atteinte ; plafond ★ N = approche sans réveil) — doc 10 §10.2/§10.3, 11 §11.7.3, 03 §3.6. **Impl. 2026-06-20 (item 2b)** : nom canon **« Le Dormeur des Fondations »** ratifié ; entrée Codex `le_dormeur` (`js/codex.js`) + 2 lignes d'ambiance nommées (zone D `before` 21+, `js/floor-ambiance.js`) ; pas de boss combat. **(2)** barks Ténébreux **étendus** : événement `loopEcho` (16 héros, `hero-barks.js`), câblé à l'affleurement d'un écho temporel (`movement.js`/`seenEchoes`) — doc 09 §VII.5, 11 §11.7.3 — **+ couche `darkBossDown`** (16 héros, clôture à la re-défaite d'un boss Ténébreux, `battle-rewards.js`) ; testé `units.js §1bis`. **(3)** biais génération par Maison **V2 OUVERT** (direction ratifiée) mais **power-neutral strict** ; **impl. = chantier suivant** (ligne ci-dessous). | ✅ | 09, 10, 11, `hero-barks.js`, `movement.js` | Variable | Fait |
| ~~**Biais de génération par Maison — V2 (implémentation)**~~ ✅ **Fait (2026-06-20)** : pondération de salles power-neutral selon `chosenHouse`, **power-neutral strict** (même grille Ch.13). **3 leviers livrés** — **(a) skin visuel** (livrée de Maison sur les cartes d'ennemi, `houseSkinClass`/`HOUSE_SKIN_ENABLED`, `battle-ui.js`) ; **(b) perception déterministe** (observation par coordonnée, `housePerceptionLine`/`floor-ambiance.js` + `movement.js`) ; **(c) pondération de salles** (le dernier) — `houseRoomBias` (`floor-ambiance.js`) réoriente la saveur du **puzzle bonus** (🦅 Serdaigle = +stèles d'énigme), câblé `dungeon.js`, gaté `houseGenBiasEnabled`. **Gate d'équité SATISFAIT** (`DIFFICULTY_REPORT.md §8`) : `P(puzzle présent)=0.44` symétrique → budget de coffres invariant (preuve par construction) ; `check_difficulty` 0 dérive ; `sim-economy` or invariant ; `scenarioHouseRoomBias` (smoke) prouve un histogramme de cellules fonctionnelles **identique** entre les 4 Maisons seed à seed ; `units.js` (invariant P=0.44). Périmètre : seul 🦅 admet une réallocation reward-équivalente (rune↔stèle) ; 🐍/🦡/🦁 restent sur la perception + le refuge commun (équité). | ✅ | 10, `floor-ambiance.js`+`dungeon.js` (salles) ; `battle-ui.js` (skin) ; `movement.js` (perception) | Élevée | Fait |
...
| ~~**Pass d'assets de fin** (illustrations victoire, SFX, fonds parchemin Codex par acte)~~ ✅ **Fait (2026-06-20)** : illustrations **victoire + Briser le Cycle** livrées (`img/scenes/ending_{victory,break_cycle}.jpg`, #596) ; **SFX « froid surnaturel »** déjà livré (`playColdBreath`/`battle.js`, testé `scenarioColdBreathSfx`) ; **fonds parchemin Codex** par acte déjà présents (`img/codex/parchment_{a,b,c,d}.png`). | ✅ | 12, 14, `audio-sfx.js`, `img/scenes` | Moyenne | Fait |
| ~~**Pass d'équilibrage de release** (`tools/sim-difficulty.js`, `check_difficulty.js` en CI)~~ ✅ **Fait (2026-06-19)** : `check_difficulty.js` était **déjà branché en CI** mais **flaky** — baseline §3 de `DIFFICULTY_REPORT.md` périmée (code actuel ~+6-9 pts aux ét. 9-12 Duo ; l'ét. 9-Duo franchissait ±10 pts à N=800, cf. flake PR #576). Baseline régénérée à **N=4000** (reflète le code, bandes saines : solo plancher 52 %, duo 72 %, aucun mur < 40 % ≤ ét. 12), §4 + exec-summary réalignés, **gate vérifié 0 dérive sur 6 runs**. Pas une régression (la doc avait pris du retard). Plan : `.claude/plans/_archive/release-balance-pass.md`. | ✅ | 13, `DIFFICULTY_REPORT.md` | Moyenne | Fait |
...
>   (`audio-sfx.js`, souffle glacé bande-passante descendante) câblé dans
>   `battle.js` à l'apparition d'un ennemi `corruption >= 2` ; testé
>   `scenarioColdBreathSfx`. **Pass d'assets de fin entièrement clos.**
...
>   la **pondération de salles** (qui changerait la distribution des types de
>   salle) reste différée derrière le gate sim de l'Item 3. N'affecte pas la démo.
