...
| **Biais de génération par Maison — V2 (implémentation)** : pondération cosmétique de salles + skins de monstres thématiques selon `chosenHouse`, **power-neutral strict** (même grille Ch.13). Gate de release : **sim d'équilibrage neutre** (`tools/sim-difficulty.js` par Maison) confirmant 0 écart de win-rate ; flag de repli V1. Spec : 10 §10.6. **Livré (2026-06-20)** : **2 leviers power-neutral** — **(a) skin visuel** (livrée de Maison sur les cartes d'ennemi, `houseSkinClass`/`HOUSE_SKIN_ENABLED`, `battle-ui.js` + CSS) ; **(b) perception déterministe** (observation propre à la Maison attachée à la coordonnée d'une salle, `housePerceptionLine`/`floor-ambiance.js` + surfaçage `movement.js`, flag `houseGenBiasEnabled`, testé `units.js` + `scenarioHouseGenBiasV2`). **Power-neutral par construction** (rendu/texte seuls, aucun `Math.random` → invisibles au sim, **0 dérive vérifiée**). **Seule la « pondération de salles »** (distribution des types de salle) reste différée derrière le gate sim (Item 3). | 🟡 Basse | 10, `battle-ui.js`+`style.css` (skin) ; `floor-ambiance.js`+`movement.js` (perception) ; `dungeon.js` (salles, gated) | Élevée | Skin + perception ✅ (2026-06-20) ; salles : sim figé |
