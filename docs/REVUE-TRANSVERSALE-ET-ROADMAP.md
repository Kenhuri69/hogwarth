...
| Tâche | Priorité | Chapitres / systèmes | Complexité | Dépendances |
|-------|----------|----------------------|------------|-------------|
| **Boss-gardiens des Chambres des Fondateurs** (ét. 17-20) + illumination selon `chosenHouse` — **DÉCOUPÉ en 3 lots** : **Lots 1-2/3 ✅ Faits (2026-06-15)**. Lot 1 = 4 boss-gardiens epic (`gardien_{lion,serpent,aigle,blaireau}`, thématisés par élément, drop = légende de Maison, `minFloor:17`, `units.js` §18). Lot 2 = placement en chambre : `_ensureChamberGuardiansPresent` (`dungeon-spawning.js`) place à l'étage 17 en Boucle les gardiens des **3 Maisons ≠ `chosenHouse`** (celle du héros l'accueille — règle d'illumination §10.5), déterministe/idempotent/gaté victoire (`scenarioChamberGuardians`). ⏳ Lot 3 = art PNG + promo/Codex. NB : illumination §10.5 + `FOUNDER_CHAMBERS` **déjà livrés** (`floor-ambiance.js`). | 🟠 Moyenne | 10, 11, `monsters.js`, `dungeon.js` | Élevée | Phase 2 |
